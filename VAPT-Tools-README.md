...
A custom web security testing suite built for Web Application VAPT engagements. Automates the tedious early-phase recon work so you can focus on actual exploitation.
...
```bash
cd BT-WebSuite
python3 bt_websuite.py --target https://target.com
```
...
python3 nessus_compliance_to_excel.py --input scan.nessus --output report.xlsx
```
...
```bash
cd Infra-VA
python3 va_automater.py --new scan_q4.xlsx --previous scan_q3.xlsx --output delta_report.xlsx
```
...
```bash
cd OSED-Automation
python3 badchar_analysis.py --binary vuln.exe --shellcode shellcode.bin
python3 skeleton_exploit.py --template seh --target 192.168.1.105 --port 9999
```
